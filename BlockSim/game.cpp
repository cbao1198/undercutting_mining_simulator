...
GameResult runGame(MinerGroup &minerGroup, Blockchain &blockchain, GameSettings gameSettings, Alpha alpha) {
    ...
    Value numUndercut = 0;
    auto lastPubTime = 0;
    while (blockchain.getTime() < gameSettings.blockchainSettings.numberOfBlocks * gameSettings.blockchainSettings.secondsPerBlock) {
        ...
        if(blockchain.getTime() - lastPubTime >= gameSettings.blockchainSettings.secondsPerBlock)
	{
	    lastPubTime = lastPubTime + gameSettings.blockchainSettings.secondsPerBlock;
	    blockchain.updateMaxHeightPub();
    ...
    Value moneyLeftAtEnd = blockchain.remFees(*winningChain[0],-1);
    
    GameResult result(minerResults, winningChain, totalBlocks, finalBlocks, moneyLeftAtEnd, totalValue,totalVariance);
